...
public class TripController {
    ...
    @Operation(summary = "Get trips. If id is provided, returns one; otherwise returns all.")
    @GetMapping
    public ResponseEntity<?> getTrips(
            @Parameter(description = "Trip id") @RequestParam(value = "id", required = false) Long id) {
    ...
    public ResponseEntity<?> searchTicketAlias(
            @RequestParam(value = "tripId", required = false) Long tripId,
            @RequestParam(value = "source", required = false) String source,
            @RequestParam(value = "destination", required = false) String destination,
            @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) @RequestParam(value = "date", required = false) LocalDate date) {
        return search(tripId, source, destination, date);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Trip> update(@PathVariable Long id, @RequestBody Trip req) {
